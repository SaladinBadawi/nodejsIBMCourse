...
//The endpoint for the webserver ending with /url/emotion
 app.get("/url/emotion", (req,res) => {
     console.log("url emotion request is triggered!!")
...
app.get("/url/sentiment", (req,res) => {
    console.log("url sentiment request is triggered!!")
     let urlToAnalyze = req.query.url
     const analyzeParams = 
        {
            "url": urlToAnalyze,
     ...
     naturalLanguageUnderstanding.analyze(analyzeParams)
       .then(analysisResults => {
        //Print the JSON returned by NLU instance as a formatted string
          console.log(JSON.stringify(analysisResults.result.keywords[0].sentiment,null,2));
     //     //Please refer to the image to see the order of retrieval
          return res.send(JSON.stringify(analysisResults.result.keywords[0].sentiment),null,2);
       })
       .catch(err => {
       return res.send("Could not do desired operation "+err);
       });
